        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, split_type: e.target.value as 'Equal' | 'No Split' })}              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
            >
    ...
    <div className="bg-white p-3 rounded-lg shadow mb-3">
      <div className="flex justify-between items-start">
        <div className="space-y-1">
          <div className="flex items-baseline gap-4">
            <div>
              <span className="text-[10px] text-gray-400 block leading-none mb-0.5">Amount</span>
              <p className="text-lg font-semibold leading-none">£{expense.amount.toFixed(2)}</p>
            </div>
            <div>
              <span className="text-[10px] text-gray-400 block leading-none mb-0.5">Date</span>
              <p className="text-xs text-gray-500 leading-none">{new Date(expense.date).toLocaleDateString()}</p>
            </div>
          </div>
          ...
          <div className="text-xs">
            <span className="text-gray-500">Paid by: {expense.users.name}</span>
            {expense.notes && <span className="text-gray-600 italic ml-2">{expense.notes}</span>}
          </div>
        </div>
      ...
      {isDeleting && (
        <div className="mt-4 p-4 bg-red-50 rounded-md">
          <p className="text-sm text-red-600 mb-2">Are you sure you want to delete this expense?</p>
          <div className="flex justify-end space-x-2">
            <button
